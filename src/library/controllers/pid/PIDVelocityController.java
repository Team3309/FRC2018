...
/*
 * PID Controller with add-ons for velocity and acceleration
...
public class PIDVelocityController extends PIDController {
	...
	public PIDVelocityController(double kP, double kI, double kD, double kV, double kA) {
		super(kP, kI, kD);
		this.kV = kV;
		this.kA = kA;
	}
	 ...
	 * */
	public void setConstants(double kP, double kI, double kD, double kV, double kA) {
	...
	public OutputSignal getOutputSignal(InputState input) {
		...
		double power = super.getOutputSignal(input).getMotor() + kV * aimVel + kA * aimAcc;
