    ...
    public void start() {
        if (onRight) {
            if (Robot.isRightSwitch()) {
                ...
                addSequential(new DriveArc(Length.fromInches(28), 82, 23000,true, true));
                addSequential(new DriveStraight(-133, 24000, true,  2.0));
                addSequential(new DriveArc(Length.fromInches(25), 80, 25000,true, true));
                addSequential(new DriveArc(Length.fromInches(25), 65, 23000,true, true));
                addSequential(new WaitCommand(0.3));
                addSequential(new ShooterShoot());
            }
        } else if (!onRight) {
