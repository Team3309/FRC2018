    ...
    public void start() {
        ...
        if (DriverStation.getInstance().getGameSpecificMessage().length() > 0) {
            if (Robot.isLeftSwitch()) {
                ...
                addSequential(new DriveArc(Length.fromInches(12), -60, 24000, true, true));
                addSequential(new DriveArc(Length.fromInches(20), 20, 24000, true, true));
                addParallel(new RollersSetIn(true));
                addSequential(new DriveStraight(35, 15000, true, true));
                addSequential(new ArmsClamp());
