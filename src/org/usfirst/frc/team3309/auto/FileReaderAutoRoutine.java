	...
	public void loadAuto() {
		...
		try {
			...
			if (!config.configurationAt(allianceColor).isEmpty()) {
				for (HierarchicalConfiguration<ImmutableNode> auto : config.childConfigurationsAt(allianceColor)) {
					
					if (!auto.configurationAt("actions").isEmpty()) {
						for (HierarchicalConfiguration<ImmutableNode> action : auto.childConfigurationsAt("actions")) {
							Class<? extends Action> newAction;
							try {
								newAction = (Class<? extends Action>) Class.forName(action.getString("action"));
					...
					if (!auto.configurationAt("driveAction").isEmpty()) {
						for (HierarchicalConfiguration<ImmutableNode> driveActions : auto.childConfigurationsAt("driveAction")) {
							...
							if (!driveActions.configurationAt("waypoints").isEmpty()) {
								...
								DrivePositionController x = new DrivePositionController(goal);
								addDriveAction(new DriveAction(x, timeout));
							}
						}
					}
